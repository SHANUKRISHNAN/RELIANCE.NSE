...
<style>
    ...
    div[data-testid="stSidebar"] button[kind="secondary"] {
        background: rgba(37,40,64,0.8) !important;
        border: 1px solid rgba(61,66,112,0.6) !important;
    ...
    div[data-testid="stSidebar"] button[kind="secondary"]:hover {
        background: rgba(61,66,112,0.7) !important;
...
with st.sidebar:
    ...
    # Toggle state stored in session_state
    if "inputs_open" not in st.session_state:
        st.session_state.inputs_open = True

    # Toggle button — arrow flips direction based on state
    arrow = "▲  Hide Inputs" if st.session_state.inputs_open else "▼  Show Inputs"
    if st.button(
        arrow,
        key         = "toggle_inputs",
        use_container_width = True,
        help        = "Show or hide the CSV upload and forecast controls"
    ):
        st.session_state.inputs_open = not st.session_state.inputs_open
        st.rerun()

    # Panel content — only shown when open
    if st.session_state.inputs_open:
        st.markdown("""
        <div style='
            background: rgba(30,34,53,0.7);
            border: 1px solid rgba(61,66,112,0.5);
            border-radius: 10px;
        ...
        st.markdown(
            ...
            " font-weight:600; margin-bottom:6px'>📂 Upload Stock CSV</div>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            ...
            "Same format as RELIANCE.csv — Date, Open, High, Low, Close, Volume"
            "</div>",
            unsafe_allow_html=True
        )

        st.markdown("<hr style='border-color:#2d2f3e; margin:8px 0'>",
                    unsafe_allow_html=True)

        # Forecast horizon slider
        st.markdown(
            "<div style='font-size:0.82rem; color:#93c5fd;"
            " font-weight:600; margin-bottom:4px'>📆 Forecast Horizon</div>",
            unsafe_allow_html=True
        )
        n_future = st.slider(
            "Days", 1, 60, 30,
        ...
        # Horizon quality indicator
        if n_future <= 5:
            horizon_color = "#34d399"; horizon_msg = "✅ High confidence"
        elif n_future <= 15:
            horizon_color = "#f59e0b"; horizon_msg = "⚠️ Moderate confidence"
        else:
            horizon_color = "#f87171"; horizon_msg = "❌ Low confidence"

        st.markdown(
            f"<div style='font-size:0.75rem; color:{horizon_color};"
            f" margin-top:2px'>{horizon_msg} · {n_future} days</div>",
            unsafe_allow_html=True
        )
        ...
        # When hidden — still need n_future and uploaded to exist
        uploaded = None
        n_future = st.session_state.get("last_n_future", 30)

    # Persist n_future so it survives collapse
    if st.session_state.inputs_open:
        st.session_state.last_n_future = n_future
